...
export function CommunityIntelligence() {
  ...
  useEffect(() => {
    if (showAskModal) {
      ...
      const t = setTimeout(() => setModalRendered(false), 300);
      return () => clearTimeout(t);
    }
  }, [showAskModal]);
    ...
    <div style={{ background: 'transparent', minHeight: '100vh', fontFamily: 'var(--font-ui)' }}>
      ...
      {modalRendered && createPortal(
        <div
          ...
          className={`fixed inset-0 overflow-hidden bg-[#0F172A]/75 backdrop-blur-xs z-50 flex items-center justify-center p-4 transition-all duration-300 ${
            modalVisible ? 'opacity-100' : 'opacity-0'
          }`}
        >
          <div 
            className={`w-full max-w-4xl bg-white rounded-xl shadow-2xl overflow-hidden border flex flex-col h-[90vh] transition-all duration-300 ${
          ...
          >
            ...
            <form onSubmit={handleAskQuestionSubmit} className="flex flex-col flex-1 overflow-hidden">
              <div
                ...
                className="p-6 space-y-4 overflow-y-auto overscroll-contain flex-1 bg-gray-50/20 no-scrollbar"
              >
